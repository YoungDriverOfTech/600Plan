...
class Solution {
    ...
    // 为什么对高度进行逆序排序，因为宽度如果一样，那么高度越高，越有可能容下前面的信封，且一旦选择了最高的，剩下的高度因为递减，所以在dp计算的时候都不会被选择
...
- 问题：有n种物品，物品i的体积为Vi，价值为Wi，每种物品只有1个，可选择放入背包，求解在背包体积限制V下可获取的价值最大
- 状态：dp[i] [j]代表前i个物品，占用j的空间所能获得的最大价值
- 转移方程：dp[i] [j]= max(dp[i - 1] [j], dp[i - 1] [j - V[i]] + W[i])
- 一维数组优化：dp[j] = max(dp[j], dp[j - V[i]] + W[i]) 
- 题目条件：1 恰好装满背包 2 无要求
- 求解： max(dp[i] [j])
...
> 对于第i件物品，可以选择拿，或者不拿。
>
> 如果选择不拿 => dp[i] [j] = dp[i - 1] [j]  在i的上一件物品的时候已经占满j空间所能获取到的最大价值
>
> 如果选择拿 => dp[i] [j] = dp[i - 1] [j - V[i]]  + W[i]  在i的上一件物品选完后，应该还剩下可以选第i件物品的空间，所以要有j - V[i]。因为选择了第i件物品，需要把第i件物品的价值加上
